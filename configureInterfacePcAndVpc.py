...
def input_key_args(msg='\nPlease Specify the Interface Profile:', delete_function=False):
    print msg
    # self.switch_profile, self.switch_selector, self.switches,
    ...
    if not delete_function:
        ...
        if len(args[0]) == 2:
            args.append("Leaf" + args[0][0] +"n" +  args[0][1]+ '-SwiPfl')
            args.append("Leaf" + args[0][0] +"n" +  args[0][1]+ '-SwiSel')
            args.append("Leaf" + args[0][0] +"n" +  args[0][1]+ '-IntSelPfl')
        elif len(args[0]) == 1:
            args.append("Leaf" + args[0][0] + '-SwiPfl')
            args.append("Leaf" + args[0][0] + '-SwiSel')
            args.append("Leaf" + args[0][0] + '-IntSelPfl')
        else:
            print 'Error too many switches!!!'
            sys.exit()
    ...
    #Build the TfNSW Syntax
    return args


def configure_interface_pc_and_vpc(infra, switch_profile, switch_selector, switches, interface_type,interface_selector_profile, interface_selector, policy_group):
    ...
    for int_sel_name in interface_selector.keys():
        ...
        infra_rsaccportp = RsAccPortP(infra_nodep, 'uni/infra/accportprof-'+ interface_selector_profile)
...
class ConfigureInterfacePcAndVpc(CreateMo):
    ...
    def delete_mo(self):
        self.check_if_mo_exist('uni/infra/accportprof-' + self.interface_selector, module=AccPortP, description='Interface Profile')
        self.mo.delete()
        self.commit_change()
